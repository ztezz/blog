              ...
              {(isGenerating || automationStatus?.progress) && (() => {
                ...
                return (
                  <div className="space-y-6">
                    ...
                    {!!diagnostics?.errors?.length && <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-500/20 dark:bg-amber-500/10"><p className="text-xs font-bold uppercase tracking-wider text-amber-800 dark:text-amber-300">Chẩn đoán gần nhất</p><ul className="mt-2 space-y-1 text-xs text-amber-700 dark:text-amber-200">{diagnostics.errors.slice(-3).map((error, index) => <li key={`${error}-${index}`} className="break-words">{error}</li>)}</ul></div>}
